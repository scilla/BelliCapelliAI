...
## Introduction
The OpenAI Realtime API enables low-latency, multimodal interactions including speech-to-speech conversational experiences and real-time transcription. This API works with models such as GPT-4o-realtime-preview.

## Creating a Realtime Session (v1/realtime/sessions)

When creating a realtime session, you need to make a POST request to the `/v1/realtime/sessions` endpoint. Here's the correct format:
...
// POST https://api.openai.com/v1/realtime/sessions
{
  ...
  // Optional - Specify voice parameters
  "voice": "alloy",
  
  // Optional - Configure RTCConfiguration parameters
  "rtc_configuration": {
    // Custom ICE servers if needed
  },
  
  // Instructions are provided as messages
  "messages": [
    {
      "role": "system",
      "content": "You are a helpful assistant that speaks clearly and concisely."
    }
  ],
...
The response from the session creation endpoint includes:

```javascript
{
  "token": "ert_abc123...",        // Ephemeral token for authentication
  "rtc_url": "wss://...",          // WebRTC connection URL
  "rtc_configuration": {             // RTC configuration for WebRTC
    "ice_servers": [
      {
        "urls": ["stun:...", "turn:..."],
        "username": "...",
...
## Implementation Example for Our Hair Salon Use Case

Here's how we should structure our request for the Belli Capelli hair salon:
...
// POST https://api.openai.com/v1/realtime/sessions
{
  ...
  "messages": [
    {
      "role": "system",
      "content": "You are a hair salon receptionist for \"Belli Capelli\" hair salon. Help clients schedule appointments, answer questions about services, and be friendly and professional. Communicate in Italian primarily."
    }
  ],
  "tools": [
    {
...
- **Unknown parameter errors**: Make sure you're using the correct parameter names according to the latest API documentation.
- **Authentication errors**: Verify your API key is correctly set and has the necessary permissions.
- **WebRTC connection issues**: Check that your ICE servers are correctly configured and accessible.

## Resources

- [OpenAI Realtime Console Demo](https://github.com/openai/openai-realtime-console)
